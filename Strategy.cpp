	...
	int lastX, int lastY, const int noX, const int noY, int &x, int &y, double value, int side, int gen, double &ans) //自己的(1 能赢 -1 投降 0 平(对自己))
{
	...
	for (int dev = 0; dev <= 2; ++dev)
	{
		...
		if (ans2[dev] == 1)
		{
			...
			else if (x != lastX + (dev - 1)*req[dev][0] || y != lastY + req[dev][0]) {
				ans = -1;
				// printf("-1muti4\n");printboard(M,N,board,top);
				return;
			}
		...
		if (ans2[dev] == 3)
		{
			...
			else
			{
				...
				for (int i = 0; i<reqf[f3]; ++i)
				{
					...
					if (j == reqf[dev]) {
						reqf[f3]--; req[f3][i] = req[f3][reqf[f3]];
						// printf("3fix\n");
					}
				}
				if (reqf[f3] == 0) {
...
extern "C" __declspec(dllexport) Point* getPoint(const int M, const int N, const int* top, const int* _board, 
	...
	if (fl1 == 0) walk(M, N, top2, board, lastX, lastY, noX, noY, x, y, value, side, gen, ans);//side为对手的代号
																							  //    printf("returned:%d %d\n",x,y);
	if (ans == -1) {
		 if ( y <0 || y >= N || top[y] == 0)do { y = (rand() + 1) % N; } while (top[y] == 0 || top[y] == 1 && y == noY && noX == 0); x = top[y] - 1; if (y == noY && x == noX) --x;
		//getchar();
	}
